...
void TestCommerceV2::apiPurchaseI(Ref* pSender)
{
	...
	IgaworksPlugin::purchase("PRODUCT_ID", 2000.00, AdBrixCurrencyKRW, AdBrixPaymentCreditCard);
#endif
}

void TestCommerceV2::apiPurchaseII(Ref* pSender)
{
	//make the extra value 
...
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
	IgaworksPlugin::purchase("PRODUCT_ORDER_ID", pProductModel.c_str(), 1000.00, 3500.00, AdBrixPaymentCreditCard);
#endif
}
...
void TestCommerceV2::apiPurchaseIII(Ref* pSender)
{
    ...
    AdBrixCommerceProductAttrModel *attr = AdBrixCommerceProductAttrModel::create(pMap);
    AdBrixCommerceProductCategoryModel *cat = AdBrixCommerceProductCategoryModel::create("cat1", "cat2");
    AdBrixCommerceProductModel *pProductModel = NULL;
    ...
    pProductMap["key01"]=pProductModel->AdBrixCommerceProductModel::create("cocos2d_x_PRODUCT_ID01", "cocos2d_x_PRODUCT_NAME01", 500.0, 100.0, 1, currencyUSD.c_str(), cat, attr).c_str();
    pProductMap["key02"]=pProductModel->AdBrixCommerceProductModel::create("cocos2d_x_PRODUCT_ID02", "cocos2d_x_PRODUCT_NAME02", 600.0, 200.0, 1, currencyUSD.c_str(), cat, attr).c_str();
    pProductMap["key03"]=pProductModel->AdBrixCommerceProductModel::create("cocos2d_x_PRODUCT_ID03", "cocos2d_x_PRODUCT_NAME03", 700.0, 300.0, 1, currencyUSD.c_str(), cat, attr).c_str();
    pProductMap["key04"]=pProductModel->AdBrixCommerceProductModel::create("cocos2d_x_PRODUCT_ID04", "cocos2d_x_PRODUCT_NAME04", 800.0, 400.0, 1, currencyUSD.c_str(), cat, attr).c_str();
    pProductMap["key05"]=pProductModel->AdBrixCommerceProductModel::create("cocos2d_x_PRODUCT_ID05", "cocos2d_x_PRODUCT_NAME05", 900.0, 500.0, 1, currencyUSD.c_str(), cat, attr).c_str();
